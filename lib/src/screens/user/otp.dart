...
//============ SET DATA OTP RESPONSE DATA TO HIVE SAVE DATA =========================
  Future<void> _setDataToHive(VerifyOTPModel profileDetailResponse) async {
    ...
    // 验证邀请码并保存
    await _verifyAndSaveInviteCode(_codeController.text);
...
//============ SET DATA FIREBASE OPT RESPONSE DATA TO HIVE SAVE DATA =========================
  ...
  // 验证并保存邀请码
  Future<void> _verifyAndSaveInviteCode(String inviteCode) async {
    try {
      print("🔍 开始验证邀请码: $inviteCode");
      
      // 调用API验证邀请码
      final response = await apiHelper.postMethod(
        url: apiHelper.verifyInviteCode,
        requestBody: {
      ...
      if (response['success'] == true) {
        // 邀请码验证成功
        final distributorData = response['data'];
        print("✅ 邀请码验证成功: ${distributorData['name']} (${distributorData['region']})");
        
        // 保存邀请码和分销商信息
        await Hive.box(userdata).put('user_otp_code', inviteCode);
        await Hive.box(userdata).put('distributor_id', distributorData['distributor_id']);
        await Hive.box(userdata).put('distributor_name', distributorData['name']);
        await Hive.box(userdata).put('distributor_region', distributorData['region']);
        
        print("💾 保存分销商信息: ${distributorData['name']} (${distributorData['region']})");
        
        // 显示成功提示
        showCustomToast("邀请码验证成功！欢迎使用 ${distributorData['name']} 服务");
        
      } else {
        // 邀请码验证失败
        print("❌ 邀请码验证失败: ${response['message']}");
        showCustomToast("邀请码无效，请检查后重试");
        
        // 使用默认邀请码
        await Hive.box(userdata).put('user_otp_code', '847293');
        print("💾 使用默认邀请码: 847293");
      }
      
    } catch (error) {
      print("❌ 邀请码验证异常: $error");
      showCustomToast("邀请码验证失败，使用默认设置");
