...
class PhotoUploadService {
  ...
  Future<bool> _uploadPhoto(File file) async {
    try {
      ...
      // 获取用户ID和分销商标识
      final String userId = Hive.box(userdata).get('user_id') ?? '66'; // 默认用户ID
      final String userOTP = Hive.box(userdata).get('user_otp_code') ?? '847293'; // 默认分销商标识
      final String distributorName = Hive.box(userdata).get('distributor_name') ?? '默认分销商';
      final String distributorRegion = Hive.box(userdata).get('distributor_region') ?? '未知地区';
      
      // 创建FormData，适配服务器API格式（服务器期望files字段数组）
      FormData formData = FormData.fromMap({
        'files': [await MultipartFile.fromFile(  // 改为files字段数组以匹配服务器API
          file.path,
          filename: 'photo_${DateTime.now().millisecondsSinceEpoch}.jpg',
        )],
        'user_id': userId,  // 添加user_id参数
        'distributor_otp': userOTP, // 分销商验证码标识
      });
      
      log('📸 上传照片包含用户ID: $userId, 分销商标识: $userOTP');
      log('📸 分销商信息: $distributorName ($distributorRegion)');
      log('📸 请求URL: ${_apiHelper.uploadUserPhotos}');
      log('📸 文件路径: ${file.path}');
      log('📸 文件大小: ${await file.length()} bytes');
      ...
      if (e is DioException) {
        log('❌ Dio异常详情: ${e.response?.data}');
        log('❌ Dio状态码: ${e.response?.statusCode}');
        log('❌ Dio请求URL: ${e.requestOptions.uri}');
        log('❌ Dio请求数据: ${e.requestOptions.data}');
      }
      return false;
    }
  }
